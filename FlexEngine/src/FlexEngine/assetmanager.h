...
#pragma region Getter
  ...
  public:
    ...
    template<typename T> static T& Get(AssetKey key)
    {
      auto asset = Internal_Get(key);
      if (asset == nullptr) throw std::runtime_error("Asset key does not exist in the asset manager: " + key);
      auto asset_ptr = std::get_if<T>(asset);
      if (asset_ptr == nullptr) throw std::runtime_error("Asset key does not match the requested type: " + key);
      return *asset_ptr;
    }
