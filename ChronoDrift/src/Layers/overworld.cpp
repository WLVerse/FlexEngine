...
namespace ChronoDrift 
{
  ...
  void OverworldLayer::Update()
  {
      ...
      // Audio system...
      for (auto& element : FlexECS::Manager::GetInstance().GetActiveScene()->CachedQuery<Audio>())
      {
        ...
        if (element.GetComponent<Audio>()->should_play)
        {
          ...
          {
            FMODWrapper::Core::PlayLoopingSound(FlexECS::Manager::GetInstance().GetActiveScene()->Internal_StringStorage_Get(*element.GetComponent<EntityName>()),
                                                FLX_ASSET_GET(Asset::Sound, AssetKey{ FlexECS::Manager::GetInstance().GetActiveScene()->Internal_StringStorage_Get(element.GetComponent<Audio>()->audio_file) }));
          }
          else
          {
            FMODWrapper::Core::PlaySound(FlexECS::Manager::GetInstance().GetActiveScene()->Internal_StringStorage_Get(*element.GetComponent<EntityName>()),
                                         FLX_ASSET_GET(Asset::Sound, AssetKey{ FlexECS::Manager::GetInstance().GetActiveScene()->Internal_StringStorage_Get(element.GetComponent<Audio>()->audio_file) }));
          }
      ...
      for (auto& element : FlexECS::Manager::GetInstance().GetActiveScene()->CachedQuery<Button, Sprite>())
      {
      ...
      // Regen Cam
      //for (auto& currCam : FlexECS::Manager::GetInstance().GetActiveScene()->CachedQuery<IsActive, Camera>())
      //{
